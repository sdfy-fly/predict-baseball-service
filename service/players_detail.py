...
class MbaPlayersDetail:
    ...
    async def get_data(self, session: aiohttp.ClientSession, url, index, d: str):
        ...
        for player in daily_data:
            ...
            if player.get('ip'):
                values_from_site = [player.get('ip'), player.get('pk'), player.get('phits'), player.get('er'),
                                    player.get('pbb'), player.get('w'), player.get('sv'), player.get('hld')]
                values_from_site = [round(float(value), 1) for value in values_from_site]
                values_from_img = [3, 2, -0.5, -2, -1, 5, 10, 5]

            else:
                values_from_site = [player.get('r'), player.get('rbi'), player.get('singles'), player.get('doubles'),
                                    player.get('triples'), player.get('hr'), player.get('bb'), player.get('k'),
                                    player.get('sb')]
                values_from_site = [round(float(value), 1) for value in values_from_site]
            ...
            if self.players.get(player["player"]):
                ...
                self.players[player["player"]] = {
                    ...
                    'team': player["team"],
                    'sum': [daily_object, *[{'match': False} for i in range(6)]]
                }
...
class NbaPlayersDetail:
    ...
    async def get_data(self, session: aiohttp.ClientSession, url, index, d: str):
        ...
        for player in data:
            ...
            values_from_site = [player['PTS'], player['REB'], player['AST'], player['STL'], player['BLK'],
                                player['BLK'], player['THREEPM'], player['TO']]
            values_from_site = [round(float(value), 1) for value in values_from_site]
